...
$query = "SELECT a.id_apre, a.nomb_apre, a.apell_apre, a.doc_apre, a.tip_doc, a.horas, a.correo, a.descrip,  a.fecha_rep, a.aprob, 
...
if (!empty($search)) {
    $conditions[] = "(a.nomb_apre LIKE '%$search%' OR a.apell_apre LIKE '%$search%'OR a.aprob LIKE '%$search%' OR u.nombre LIKE '%$search%' OR p.programa_nom LIKE '%$search%' 
...
// Verificar si se ha enviado un formulario para cambiar el estado
if (isset($_POST['aprobar']) || isset($_POST['rechazar'])) {
    $id_apre = $_POST['id_apre'];
    // Cambiamos 'aprob' a 'aprobar' para que coincida con los valores en la base de datos
    $nuevo_estado = isset($_POST['aprobar']) ? 'aprobar' : 'rechazar';

    // Aquí actualizamos el estado en la base de datos
    $sql_update = "UPDATE aprendiz SET aprob = '$nuevo_estado' WHERE id_apre = $id_apre";
    ...
    <div class="card shadow mb-4">
             ...
             <div class="card-header py-3">
                 ...
                 <!-- Opciones de cantidad de registros por página -->
                   ...
                   <h6 class="m-0 font-weight-bold text-primary">Aprobación de aprendices</h6>
                       </div>
                         <div class="card-body"> 
                        ...
                        if (mysqli_num_rows($result) > 0) {
                            while ($data = mysqli_fetch_assoc($result)) { ?>
                                <tr>
                                    ...
                                    <td class="truncated"><?php echo $data['doc_apre']; ?></td>
                                    <td class="truncated" ><?php echo $data['horas']; ?></td>
                                    <td class="truncated"><?php echo $data['programa_nom']; ?></td>
                                    <td class="truncated" ><?php echo $data['num_fich']; ?></td>
                                    <td class="truncated"><?php echo $data['fecha_rep']; ?></td>
                                    <td class="truncated" ><?php echo $data['des_novedad']; ?></td>
                                    <td class="truncated"><?php echo $data['estado_apren']; ?></td>
                                    <?php if ($_SESSION['rol'] == 1 || $_SESSION['rol'] == 3) { ?>
                                        <td class="truncated">
                                        <form method="POST">
                                            <input type="hidden" name="id_apre" value="<?php echo $data['id_apre']; ?>">
                                            <button type="submit" name="aprobar" class="btn btn-success">Aprobar</button>
                                            <button type="submit" name="rechazar" class="btn btn-danger">No Aprobar</button>
                                        </form>
                                ...
                                <tr class="details-row"> <!-- Hidden row for details -->
                                    <td colspan="15">
                                        <div>
                                            ...
                                            <p><strong>Descripción:</strong> <?php echo $data['descrip']; ?></p>
                                            <p><strong>Aprobado:</strong> <?php echo $data['aprob']; ?></p>
                                        </div>
                                    </td>
                                </tr>
                            <?php }
